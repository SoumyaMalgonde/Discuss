...
function Voting(props) {
    ...
    const total = commentsTotals && comment_id in commentsTotals
        ? commentsTotals[comment_id] : 0;

    const userVote = userVotes && comment_id in userVotes
        ? userVotes[comment_id] : 0;
    ...
    function sendVote(direction = 'up') {
        ...
        const url = 'http://localhost:4000/vote/' + props.comment_id + '/' + direction;
    ...
    function arrowButton(directionName = 'up') {
        ...
        if (directionNumber === userVote) {
            classNames += ' text-discuss_red';
        } else {
            classNames += ' text-discuss_text-darker hover:text-white';
        }
